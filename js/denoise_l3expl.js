...
document.addEventListener("mouseup", (e) => {
    if (window.latentDenoiserL2Expanded && window.latentDenoiserL3Expanded) {
        ...
        else if (e.clientX > mainLeft && e.clientX < mainRight && e.clientY > mainTop && e.clientY < mainBottom) {reduceLatentDenoiserL3(); reduceLatentDenoiserL2();}
        else {}
    }
})
let sliderWidth = 250;
...
d3.select("#latent-denoiser-l3-expl-text-container")
    .append("div")
        .text("UNet如何预测噪声?(How does UNet predict noise?)")
d3.select("#latent-denoiser-l3-expl-text-container")
    .append("div")
        .text("(1)基于空白提示词的通用噪声(generic noise conditioned on empty prompt)")
d3.select("#latent-denoiser-l3-expl-text-container")
    .append("div")
        .text("(2)基于你的提示词预测的噪声(noise conditioned on your text prompt)")
        .style("padding-bottom", "8px")
d3.select("#latent-denoiser-l3-expl-text-container")
    .append("div")
        .html('为了生成更好地符合你的提示词的图像，Stable Diffusion计算这两种噪声的加权和，权重由<span style="font-weight:500">提示词强调系数</span>控制。提示词强调系数越大，生成的图像越符合提示词的描述。')
